breed [nodes node]
breed [ants ant]
links-own [len ph visits best? local-best?]
...
to setup
  ...
  create-ants ants-number [set hidden? not hidden?]
  reset-ticks
end
...
to setup-node
  ...
  if layout = "two-circles" [
    ...
    [
      let a 360 / (graph-size / 2 + graph-size mod 2) * who
      let r max-pxcor - 1
      setxy (r * sin a) (r * cos a)
    ]
  ]
  if layout = "star" [
    let a 360 / graph-size * who
    let r 0
    ifelse who mod 2 = 0 [set r max-pxcor - 3]
    [set r max-pxcor - 1]
    setxy (r * sin a) (r * cos a)
  ]
end
...
to check-best
  ...
  if bp >= best-len [
     ask links [set local-best? false]
    ask [visited-links] of best-ant [set local-best? true]
  ]
end
...
to color-link
  ifelse show-best? [
    ask links [
      ...
      set hidden? not best?
      if mark-cur-best? [
        set hidden? not best? and not local-best?
        if local-best? [
          set thickness 0.1
        ]
        if best? and local-best? [
          set color turquoise - 3
          set thickness 0.25
        ]
      ]
    ]
  ...
  [
    ask links [
      ...
      set thickness 0.6 * visits / ants-number
...
to-report min-it
  report [path-len] of min-one-of ants [path-len]
end

to-report mean-it
  report mean [path-len] of ants
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
layout
layout
"random" "circle" "grid" "circle-center" "two-circles" "star"
3
...
"" ""
PENS
"best" 1.0 0 -955883 true "" "plot best"
"min" 1.0 0 -13791810 true "" "plot min-it"
"mean" 1.0 0 -14439633 true "" "plot mean-it"
