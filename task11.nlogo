...
to setup-patch
  ...
  [
    ...
    if init-state = "random" [
      ifelse random-float 1 < probability-1 [set state 1][set state 0]
    ]
    if init-state = "cluster" [
      let half-min -1 * floor(cluster-size / 2)
      let half-max ceiling(cluster-size / 2)
      ifelse pxcor > half-min and pxcor < half-max and random-float 1 < probability-1
      [set state 1][set state 0]
    ]
